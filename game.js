...
canvas.height=600;
	...
	if (document.getElementById('class').value=="mage") hero.src = 'img/wizard.png';
		if (document.getElementById('class').value=="fighter") hero.src = 'img/fighter.png';
			if (document.getElementById('class').value=="rogue") hero.src = 'img/rogue.png';
...
{//alert(mapArray[2]);
	...
	for (var x=0; x < worldWidth; x++)
	{
		...
		{
			world[x][y] = parseInt(mapArray[x+(y*worldWidth)]);
...
*/
	...
	//ctx.drawImage(hero,CHAR_WIDTH*1,CHAR_HEIGHT,CHAR_WIDTH, CHAR_HEIGHT,	 currentPath[rp][0]*tileWidth-tileWidth,	currentPath[rp][1]*tileHeight-70,CHAR_WIDTH, CHAR_HEIGHT);
...
function redraw()
{
	...
	ctx.fillRect(-300, -300, canvas.width+300, canvas.height+300);
...
var img = document.getElementById("background");
    ctx.drawImage(img, 0, -0,2000,1800,0,0,worldWidth*tileWidth,worldHeight*tileHeight-300);
...
for (var x=0; x < worldWidth; x++)
	...
	/*	
if (spriteNum==1) ctx.drawImage(spritesheet,
			zoom*spriteNum*tileWidth, 0,
			zoom*tileWidth,zoom*tileHeight,
			zoom*x*tileWidth-5,zoom*y*tileHeight,
			zoom*tileWidth, zoom*tileHeight);	*/		
		}
	}
	GAME_FONTS = "bold 30px sans-serif";
ctx.fillStyle = "#000";
ctx.fillText("x:"+x+" y:"+y, 100, 20);
...
smer=0;
	 if (rp>0) 	 {
/*	 ctx.drawImage(hero,CHAR_WIDTH*8, CHAR_HEIGHT,CHAR_WIDTH, CHAR_HEIGHT,	currentPath[rp-1][0]*tileWidth-30,	currentPath[rp-1][1]*tileHeight-70,CHAR_WIDTH, CHAR_HEIGHT);*/

	 if (currentPath[rp-1][0]<currentPath[rp][0]) {smer=1;}//if going right sprite=image right
	 if (currentPath[rp-1][0]>currentPath[rp][0]) {smer=3;} //if going left sprite=image left
	 if (currentPath[rp-1][1]<currentPath[rp][1]) {smer=2;} //if going down sprite=image down
	if (currentPath[rp-1][1]>currentPath[rp][1]) {smer=0;} //if going up sprite=image up
	
	 }

	if (spriteNo==7) spriteNo=0;
	if (spriteNo<8) spriteNo=spriteNo+1;
	

	

	 ctx.drawImage(hero,CHAR_WIDTH*spriteNo,CHAR_HEIGHT*smer,CHAR_WIDTH, CHAR_HEIGHT,	 currentPath[rp][0]*tileWidth-tileWidth,	currentPath[rp][1]*tileHeight-70,CHAR_WIDTH, CHAR_HEIGHT);
 
		rp++;
	
		
	 if (currentPath[rp-1][0]<currentPath[rp][0]) { ctx.translate(tileWidth*(-1), 0);translateX++;}//if going right translate right
	 if (currentPath[rp-1][0]>currentPath[rp][0]) {ctx.translate(tileWidth, 0);translateX--;} //if going left  translate left
	 if (currentPath[rp-1][1]<currentPath[rp][1]) {ctx.translate(0, tileHeight*(-1));translateY++;} //if going down  translate down
	 if (currentPath[rp-1][1]>currentPath[rp][1]) {ctx.translate(0, tileHeight);translateY--;} //if going up  translate up
		//alert(translateX+","+translateY)
		
	if (rp<currentPath.length) {  
	setTimeout(drawCharacter,100)	;}
...
function canvasClick(e)
{
	...
	var cell =
	[
	Math.floor((x/tileWidth)/zoom)+translateX,
	Math.floor((y/tileHeight)/zoom)+translateY
	];
if (world[cell[0]][cell[1]] == 1) return 0;
	// now we know while tile we clicked
//	alert('we clicked tile '+cell[0]+','+cell[1]);
herosound();
	pathStart = pathEnd;
	pathEnd = cell;
...
var audio = document.getElementById('herosnd');
    audio.src = 'snd/warrior'+rand+'.ogg';        
    audio.load();
	audio.play();
}